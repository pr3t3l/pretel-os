# tasks.archive.md — Completed atomic tasks (read-only history)

**Status:** Append-only archive
**Purpose:** Grep-able historical record of atomic-task detail that was rolled out of the live tasks.md when modules closed. Original work history is also in `git log`; this file is the convenience copy.

**Convention:** When a module's phase closes, a closure summary (commits + artifacts + atomic count) is appended here under a dated section header. Full atomic-level detail (one line per migration, function, test) stays canonical in per-module `specs/<module>/tasks.md`. The pre-migration snapshot below preserves the verbatim root tasks.md content from before the milestone-only refactor — that pre-rule content has no per-module home and lives only here + in git history.

---

## Pre-migration snapshot — 2026-04-28 (verbatim root tasks.md before milestone-only refactor)

Full content of `tasks.md` as it existed at commit 12b3d1f, immediately before the Opción B refactor that converts root tasks.md to milestone-only and moves atomic detail into per-module `specs/<module>/tasks.md`.

Modules 1-3 closed before the SDD trinity rule existed (their atomic detail has no per-module home); their content below is the only readable record outside git log archaeology.
...
## Module 0.X — Phase A (closed 2026-04-28)

Source: `specs/module-0x-knowledge-architecture/tasks.md` (M0X.PRE.* + M0X.A.*)

**Closure commits:**
- db73a67 — M0X.A.1 migration 0024 tasks
- 394bf1a — M0X.A.2 migration 0025 operator_preferences
- fe923a9 — M0X.A.3 migration 0026 router_feedback + spec §5.4 amendment
- 885adba — M0X.A.4 migration 0027 best_practices (HNSW deferred per ADR-024)
- cb56311 — fix(M2): notify_missing_embedding polymorphic CASE bug (0028a)
- acac675 — M0X.A.5 migration 0028 decisions amendment + spec §5.2 fix
- 40d51cc — M0X.A.6 migration 0029 data migration (closes Phase A)
- 3e55baf — M0X.A.7 post-migration schema audit
- bc4e5df — M0X.A: mark Phase A tasks complete

**Schema artifacts produced:**
- 4 new tables: tasks, operator_preferences, router_feedback, best_practices
- decisions table amended with 7 columns
- 5 ADRs seeded (020-024)
- 4 misclassified lessons archived with cross-table pointers
