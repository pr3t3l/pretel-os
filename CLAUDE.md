...
**Five background workers (CONSTITUTION §2.6):** Reflection (M6, cancelled — see decision row `9e8bacad`), Dream Engine (nightly cron, live as `pretel-os-dream-engine.service`), Morning intelligence (n8n, planned), Auto-index on save (live as `pretel-os-autoindex.service` since 2026-05-07 — LISTENs on `embedding_queue`, drains `pending_embeddings`), README consumer (M7.5, live as `pretel-os-readme.service`). The MCP process also hosts a daemon thread for `LayerBundleCache` LISTEN on `layer_loader_cache` (M4 / 0031) — process-internal, not a separate worker.
