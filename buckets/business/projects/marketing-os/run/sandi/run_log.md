...
| Phase 2 | G-Phase-2-PRE | ✅ passed 2026-06-12 (13/13 aplicables + check 13 = el propio 2.0) | checks 1-5 Phase 1 ✓ (statement 310 pal., language_pack 7/5, handoff 4 campos) · 6 ✓ awareness 5/20/50/25=100 · 7 ✓ con NOTA (channel=online_only es scope, no lista — canales concretos salen de hábitos del avatar en 2.2, gap Pattern C) · 8-9 ✓ · 10 N/A (hybrid B2B parked, criterio D-022/23) · 11-12 ✓ (force_coverage + displacement firmados) · 14 ✓ strat_priya_v1 |
| Phase 2 | 2.0 Brand Voice | ✅ **CERRADO (D-040)** | `brand_voice.json` — sage+caregiver ("el mentor que se queda") citando JTB-1 literales (regla R4) · CPM · 5 tone descriptors · lexicon 14/18 + 4 example_pairs · 5 consistency rules testeables · 0 conflictos con language_pack de Priya. DECISIÓN IRREDUCIBLE DEL OPERADOR |
| Phase 2 | 2.1 Content Mix | ✅ **CERRADO (D-041)** | `content_mix.json` — Caso B (create_demand) modulado por demand_type=mixed de Priya: **15/30/40/15** (todas las bandas B ✓; delta máx 10 ≤30, CONTENT-004 no dispara) |
| Phase 2 | 2.2 Canal×Momento | 🔄 DRAFT esperando validación | `channel_journey_matrix.json` — 5 canales (ENMENDADA D-042: el operador corrigió la exclusión de IG/TikTok — "aquí vive todo"; grandes superficies ON por defecto, rrss_social entra texto-primero con carruseles de plantilla) · seo_blog · comunidades value-first · email · rrss_social 2/sem · paid_search SOLO-ventana deferred · excluidos CON caso: Pinterest (superficie del comprador-de-Priya) / YouTube (degradado: guiones listos V2) / LinkedIn (B2B parked) · carga ~6 acciones/sem con válvula de degradación |
| Phase 2 | 2.3–2.5 | ⏳ | 4 pilares por fuerza (REINFORCE vs RESOLVE) → atomización → hooks | mix por awareness (create_demand + demand_type=mixed de Priya) → matriz canal×journey → 4 pilares por fuerza (REINFORCE vs RESOLVE) → atomización → hooks |
