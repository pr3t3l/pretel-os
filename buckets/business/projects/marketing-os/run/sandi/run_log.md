...
| Phase 2 | 2.0 Brand Voice | 🔄 DRAFT esperando decisión | `brand_voice.json` — sage+caregiver ("el mentor que se queda") citando JTB-1 literales (regla R4) · CPM · 5 tone descriptors · lexicon 14/18 + 4 example_pairs · 5 consistency rules testeables · 0 conflictos con language_pack de Priya. DECISIÓN IRREDUCIBLE DEL OPERADOR |
| Phase 2 | 2.1–2.5 | ⏳ | mix por awareness (create_demand + demand_type=mixed de Priya) → matriz canal×journey → 4 pilares por fuerza (REINFORCE vs RESOLVE) → atomización → hooks |
