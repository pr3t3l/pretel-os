...
| Phase 2 | 2.1 Content Mix | 🔄 DRAFT esperando firma | `content_mix.json` — Caso B (create_demand) modulado por demand_type=mixed de Priya: **15/30/40/15** (todas las bandas B ✓; delta máx 10 ≤30, CONTENT-004 no dispara) |
| Phase 2 | 2.2–2.5 | ⏳ | mix por awareness (create_demand + demand_type=mixed de Priya) → matriz canal×journey → 4 pilares por fuerza (REINFORCE vs RESOLVE) → atomización → hooks |
