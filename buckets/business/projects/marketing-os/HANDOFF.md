...
- ⏭️ **Phase 0.3 (Persona + Avatares) — RETOMAR AQUÍ, con giro en curso.** Al abrir 0.3 el operador rechazó el corte por profesión y propuso recorte **situacional** del beachhead: dueños de **oferta propia (producto O servicio)** que intentan venderla online sin saber cómo (empezar/escalar). **Enmienda D-022 pendiente de confirmar** (v2: `own_offer_ready` + `multi_audience` + `online_native_diy`; deal-breakers iguales). Avatares candidatos: "Lanzador" (cero ventas, ¿cómo empiezo?) / "Estancado" (vende, ¿cómo crezco?) — pasan 2-de-3. Si confirma: `decision_supersede` D-022→D-023 + icp.json v2 + re-firmar gate. Arrastra: `negative_personas` + priorización de primer ciclo. Reglas spec §6: pre-PMF ⇒ 1 persona primario; avatares 2–4; test 2-de-3 (trigger/canal/lenguaje).
