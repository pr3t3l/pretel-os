...
- **BUILD EN CURSO (carril a, mismo chat — decisión del operador 2026-06-10):** mandato = UX como goal principal ("como los iPhone"; cada página con lo necesario para seguir el flow entre fases). Nuevos specs committeados: `spec_UX_Experience.md` (7 principios con ciencia + patrón de pantalla canónico + motion tokens + anti-patrones + métricas) y `build_plan_phase0_wizard.md` (inventario real del repo, fusión mockup-Claude-Design + 6 movimientos = "wizard conversado", M1–M6, decisiones abiertas B1–B3). Diseño visual = zip `Sandia Marketing.zip` (chrome rail/stepper/panel + tokens watermelon dark, screenshots). Pendientes que siguen: (b) formalizar guiones 0.2–0.4 + CAG/evals; (c) sim Phase 1.
