...
3. **¿Transiciones clip 1→2→3→4?** — hoy son **implícitas** (misma descripción de personaje/set/voz en cada clip = continuidad; Kling empaqueta ≤15s en `multi_prompt`). El **frame chaining** (último frame del clip N → `start_image` del N+1, vía FLUX Kontext) es la mejora PLANEADA para continuidad perfecta — no construida.
