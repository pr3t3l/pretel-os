...
**D-033 adds Module C — Admin & Cost Intelligence** (`spec_Admin_Cost_Intelligence.md`): the operational, NON-user-facing module — admin console, continuous cost telemetry (per user/phase/model/action-class), satisfaction signals (card accept/adjust/reject rates), model-routing control, support mode (audited), billing/credits, and the system-level learning loop. It calibrates Module B's credit pricing with real data and fills Module A's cost structure with measurements instead of hypotheses.
...
3. **Seasonal window of a benched avatar** — dated, reasoned suggestion (e.g. Héctor's spring: "to capture his season, light him up in February").
4. **`avatar_underperforming`** — pause one, reallocate focus/credits to the next benched avatar.
5. **Credit headroom** — the user's plan absorbs another active avatar without strain.

Recommended default (mirrors the operator's own D-026 discipline): **one avatar until the first loop closes.** The "Encender" action lives in the project hub (UX spec §2c), not buried inside step 0.3.
...
─ multi_avatar_strategy text             -- CONTENT/distribution granularity only: single_avatar | unified_C_* | separate_strategies. NEVER an offer split — the offer is the buyer persona's (one shared offer_spec for all avatars, C5); this field only groups how the per-avatar CONTENT strategy is authored.
...
─ offer_spec_id       uuid           -- Phase 1 output (offer_spec.json) — the ONE buyer-persona offer, shared by all of the persona's avatars (C5); strategies of different avatars under the same persona reference the same offer_spec, never per-avatar offers
...
**Content-strategy granularity — unified vs separate (resolves the multi-avatar tension):** this axis groups how the **content/distribution strategy** is authored per avatar. It is **never an offer split** — the offer is the buyer persona's, ONE shared `offer_spec` referenced by every strategy under that persona (C5). What branches here is content, not the offer.
- **`separate_strategies` (default):** one content strategy per avatar, all over the SAME shared offer. `avatar_id` set, `covers_avatar_ids = [avatar_id]`. This is the clean per-avatar case from the diagram — N avatars → N parallel content strategies on one offer.
- **`unified_C_*`:** one content strategy serves several near-identical avatars (a Phase 2 content-unification optimization, still on the one shared offer). `avatar_id` = primary avatar, `covers_avatar_ids` = all covered avatars. The shared strategy still produces per-strategy results/learnings, but **content** is differentiated per avatar via `language_packs` inside the single plan — the offer stays identical.
- **`single_avatar`:** trivially one avatar.

**Invariant:** each avatar is served by exactly one `active` content strategy at a time (either its own, or a shared unified one), all riding on the buyer persona's single offer. Phase 5 creates the next version and supersedes the previous. Results, learnings, decisions and best practices are queried per `strategy_id`.
...
- `spec_Phase_1_Oferta.md` — offer construction (+ strategy birth, D-009/010). **v1.8**: valor por FUNCIONALIDAD (sin perceived_value/ratio), único candado = margen por delivery_format, eje débil por mecanismo + reversión de riesgo (decision `a2ef7e37`).
...
- D-006 (2026-06-01) — Stack MVP locked as Next.js + Supabase Cloud + Vercel. *(VIGENTE — "Stack" aquí = stack TÉCNICO de infraestructura; no confundir con el "offer stack" de oferta, cuyo modelo de valor por comparación/ratio fue retirado en v1.8 / decision `a2ef7e37`.)*
...
- D-038 (2026-06-12, DB `308fb419`, **critical**) — *(SUPERSEDED por v1.8 / decision `a2ef7e37` — el offer_spec firmado se describió bajo el modelo viejo de value-stack/ratio; la doctrina vigente valora por FUNCIONALIDAD y el único candado es margen por delivery_format. La fila se conserva como registro histórico del cierre de Fase 1; no reintroducir perceived_value/ratio.)* — **Sandi run: PHASE 1 CLOSED — global gate G-Phase-1 PASSED.** offer_spec signed (strat_priya_v1 / **Papandi** / The Marketplace Independence Plan): $30/mo single-tier + free door (0.1 cowork) + Beats allowance (1/5/60 seed weights), unconditional 30d guarantee, honest Q4 deadline with retirement rule, EN statement (~310 words) built 100% from signed pieces. papandi.com bought, assigned and verified serving prod (operator did purchase+assignment same hour). Metadata ~5h/~$15 [estimado]. Second validation of the sim-first pipeline (D-024): 8 decisions, 8 artifacts, 3 USER-CORRECTED doctrines into canon, 1 new module (C), 1 definitive name — one session. NEXT (one-phase-lag): Phase 2 (Contenido) sim while build takes Oferta step + Papandi rebrand + hub. Launch gates pending (non-blocking): TM screen e66dce5a, rebrand 942d5214, Beats system 5953b520. Lesson 286c1222 auto-approved.
- D-037 (2026-06-12, DB `98f204a2`, **critical**) — **THE DEFINITIVE NAME: PAPANDI** (operator-coined: PAPaya + sANDÍa — his daughter's two fruits; EN-trivial "pa-PAN-dee"). "Sandi" retired as working name — indefensible in US: Santander owns SANDI class-042 (SaaS chatbot NLP, serial 90359826) + 2024 filing; VORTO class-9 (2019); Sandy AI (Salesmate) phonetic twin in sales/CRM. Papandi screen: ZERO web results; papandi.com + papandi.ai both free at decision time; .com purchase authorized (interactive handoff — Vercel CLI requires confirmation); .ai recommended defensive grab pending. **Family story stays PRIVATE** (permanent brand rule: public copy never uses it). name_status working→final in business_context (the §4b two-step promotion, recorded amendment). Launch gates (don't block build/sim): formal USPTO+lawyer screen (task e66dce5a) · product rebrand Sandi→Papandi (task 942d5214). Sapandi discarded ("sap" = fool in EN slang); Pandi discarded (taken roots + P&I marine insurance).
- D-036 (2026-06-11, DB `206ba1cc`) — *(SUPERSEDED por v1.8 / decision `a2ef7e37` — el re-score a 1440 se calculó como composite con ratio de valor; la doctrina vigente mide la fuerza por el MOVIMIENTO DEL EJE DÉBIL (`proposed_rescore`), no por un ratio. La garantía/urgencia honestas y el displacement siguen vigentes; el número-ratio no.)* — **Sandi run: G-Phase-1.3 SIGNED ("1a 2a 3ok 4ok") + re-score to 1440.** Unconditional 30-day money-back ("no forms, no fine print" — post-guru avatar punishes conditions); seasonal Q4 deadline urgency anchored to Priya's real trigger #3 WITH retirement rule (outside jun-sep window it leaves the copy); no scarcity (don't stack pressure on a burned avatar); displacement framing approved (habit #1 cage-is-comfortable → "replaces the hours you give the algorithm" + cost-of-staying → Phase 2 central copy). Likelihood re-scored 4→6 per D-031/D-035 plan: composite 8×6×5×6 = 1440 (estándar operativo) — the offer EXITS the VALUE-002 soft block by design, not makeup. Founders-price urgency deferred (requires operator's real commitment to raise).
- D-035 (2026-06-11, DB `12c624b4`) — *(SUPERSEDED por v1.8 / decision `a2ef7e37` — el stack se aprobó con `comparable`/perceived value, ratio 11.1×≥5 y la forma canónica "Total value $X / Your investment $Y"; TODO eso se retiró. Doctrina vigente: cada pieza se justifica por lo que HACE / el freno que quita, único candado = margen por delivery_format (76%≥70% sobrevive como gate de margen; el target fijo 70% ahora se mapea por formato). El gate de fuerza ≥1 anxiety_reduce + ≥1 habit_break sigue vigente; ratio/perceived value no.)* — **Sandi run: G-Phase-1.2 SIGNED — Priya offer stack approved as proposed** (core "Marketplace Independence Plan" $99 comparable M1 + 4 bonuses with cited comparables; ratio 11.1×≥5 ✓, margin 76%≥70% ✓; ≥1 anxiety_reduce + ≥1 habit_break ✓) after instrument-education repair (stack concept taught: perceived value ≠ price; canonical form "Total value $X / Your investment $Y"; sales-page preview made it land). Beats validated as credit-unit mental model (operator-coined name) + anti-dark-pattern guardrail on suggestion loop.
- D-034 (2026-06-11, DB `ff20990b`) — *(PARCIALMENTE SUPERSEDED por v1.8 / decision `a2ef7e37` — solo el punto (6): el offer_stack se entregó con "ratio 11.1×, margin 76%" bajo el modelo viejo; vigente = valor por funcionalidad, único candado = margen por delivery_format, fuerza = `proposed_rescore` del eje débil (no ratio). El resto de la fila — política de activación de avatares, project hub, renames, cycle scope text-only, Module C — sigue VIGENTE.)* — **Operator mandates batch (session 3, round 5).** (1) Avatar activation policy SIGNED → §Strategy Lifecycle: bench + "Encender este público" + signal-based suggestions (first-loop-closed / stability / seasonal window with date+reason / underperforming-reallocates / credit headroom) + default one-avatar-until-first-loop. (2) **Project hub mandated** (UX spec §2c): avatars front and center with activation, key signed decisions visible (ideal customer, suggested price, positioning, name), phase 3-5 metrics/results — build item. (3) Renames live in prod (sandia 317acef): "Tu puerta"→"Tu cliente ideal", "Tus personas"→"Tus avatares"; naming doctrine: common marketing terms > invented metaphors. (4) **Cycle scope USER-CORRECTED**: strategy + TEXT (copy/scripts/briefs); image/video generation NOT included — user's own assets or future premium (multimodal parked 0.1); offer copy never implies finished visuals. (5) Module C parked as formal task (156e9c29, high). (6) Core deliverable approved → Priya offer_stack draft delivered (ratio 11.1×, margin 76%, both gates pass; likelihood re-score 4→6 proposed pending 1.3).
- D-033 (2026-06-11, DB `ad16fab3`) — **Module C anchored: Admin & Cost Intelligence (spec v0.1) + audience-layers doctrine + continuous cost-quality telemetry.** Operator's held-back mandate opened formally: admin console (consumption metrics, visible system prompts, model routing visible AND selectable, audited per-account support, live AI spend by system/phase, avg cost per phase, pause/delete accounts, revenue, payment profiles, system-level lessons/decisions/BPs) + telemetry that is CONTINUOUS across all users (min/max/avg, cost ranges vs quality/satisfaction vs selected LLM; card accept/adjust/reject rates + likes/dislikes at every stage; pattern-discovery-first dashboards → the money-quality optimum). Internal decisions C-D1..C-D5. Audience layers fixed in UX spec: plain flow → support panel → technical admin; technical machinery NEVER reaches user layers (test: if Dana wouldn't understand it, not layer 1). Pending before code: dedicated ideation + market research (Langfuse/Helicone/LangSmith/Portkey as LLM-ops references) + build-vs-integrate + own trinity. Phase 1 sim NOT blocked.
- D-032 (2026-06-11, DB `3bac3de4`) — *(PARCIALMENTE SUPERSEDED por v1.8 / decisions `a2ef7e37` + `11b919ab` — "el 1.2 stack está diseñado alrededor de PROOF / proof-weak ×3 → diseñar alrededor de PROOF" refleja el modelo viejo: producto pre-lanzamiento NO usa prueba social (C4); el eje débil se ataca por MECANISMO ("por qué te funciona a TI", anclado en los diferenciadores firmados) + REVERSIÓN DE RIESGO, nunca testimonios/casos. La decisión multi-avatar `separate_strategies` se conserva pero se lee como granularidad de CONTENIDO sobre UNA oferta compartida del buyer persona (C5), no como ofertas separadas. La elección `demand_type=mixed` y el nacimiento de strat_priya_v1 siguen vigentes.)* — **Sandi run: G-Phase-1.1 + G-Phase-1-multi-avatar SIGNED — first strategy born.** Set signed: Dana 1260 / Marcus 1680 (estándar, weakest=likelihood) / Héctor 672 (débil, weakest=effort — the equation CONFIRMS D-026 "good 4th, bad 1st" with math; his soft-block decision_record defers to activation). Multi-avatar: ≥2 hard conditions fail (same functional job — one persona — but anxieties 1/3, habits 1/3, vocabulary ~20%) → **separate_strategies**: the run's own evidence confirms the D-009 thesis. **strat_priya_v1 born** (v1, active, demand_type=**mixed** — first real use of clause E1: rage windows with exact search = capturable + education playbook = generated). Cross-avatar insight: likelihood-weak ×3 is structural (pre-PMF, no social proof) → the 1.2 stack is designed around PROOF. Operator also elevated to mandate: the instrument education delivered in-chat enters the system LITERALLY (spec §3 canon + sandia `value-equation-education.ts`).
- D-031 (2026-06-11, DB `00d0cd0c`) — *(PARCIALMENTE SUPERSEDED por v1.8 / decisions `a2ef7e37` + `11b919ab` — el composite "960" y el re-score por axis se conservan como el método de eje débil (la fuerza se mide por `proposed_rescore` del eje, no por ratio de valor — eso sí sigue), pero "el 1.2 stack ataca proof/trust" se reencuadra: el eje débil se ataca por MECANISMO + reversión de riesgo, nunca por prueba social (C4). La nota "HF dogfood = worked example/live demo, NOT Priya-type seller social proof" ya es consistente con C4 y sigue vigente.)* — **Sandi run 1.1: Priya value equation SIGNED at 960 (débil / soft-block VALUE-002 sanctioned).** Scores 8/4/5/6 (dream/likelihood/time/effort) signed as proposed ("procede"). Weakest_axis = likelihood (tiebreak over time 5: D-026 names post-guru skepticism as the avatar's #1 risk — the trust axis rules). Phase_1_focus = the 1.2 stack is designed to attack proof/trust (fast measurable first win, honorable guarantee in 1.3, glass-box as a selling feature); re-score after the stack closes — if it doesn't raise the axis, THAT is the real alarm. Open amendable judgments: HF dogfood counts as worked example/live demo, NOT Priya-type seller social proof (likelihood stays 4); dream statement balances rage (saying no to Amazon) with the signed measure (30% own channel).
- D-030 (2026-06-11, DB `a6d5f7ae`) — **Phase 1 spec validated vs corpus (closes the R5 loose end of D-021).** The original pass's Phase 1 agent failed; this validation closes it against Curso 2 (C5/C6/C7), BMC (PV 11 elements, revenue mechanisms, FREE patterns) and Curso 1. Verdict: superset, 8.5, zero hard contradictions. Strongest anchors: urgency-aligned-to-trigger (ISSUE-A4) = C6 activador-as-moment; Drive 3-traits = C5 relevante/diferente/creíble (two independent sources converge); margin+ratio gates = "CPA < margen". Applied as spec v1.6: E1 `demand_type` populated at strategy birth (real cross-doc drift caught — FLAG-1/D-021 added the column to `strategies` after Phase 1 v1.5 was written), E2 Osterwalder 11-value checklist for the stack, E3 `price_mechanism` menu [Extensible Vocabulary], E4 `monetization_pattern_echo` with freemium/bait-hook economics, E5 lead-magnet/free door as declared entry tier, E6 cheap-lever note + review-mining for value_rationale. No new FLAGS (FLAG-7/FLAG-8 remain parked). Detail: `corpus_validation_report.md` §Addendum 2026-06-11.
- D-029 (2026-06-10, DB `56c74af5`) — **Sandi run: PHASE 0 CLOSED — global gate G-Phase-0 passed, brief signed, build trigger D-024 FIRED.** Operator signed `product_brief_v2.json` after reviewing the rollup's new content (economics CAC auto:ltv/3 = $165 with honesty note; 3 real refuted hypotheses, 0 critical; estimated metadata 8h/$50). All sub-gates passed (D-023, D-025–D-028). Per D-024: build mode active in `sandia-marketing` (Phase 0 wizard slice) while the sim continues with Phase 1 (one-phase-lag pipeline). Cheap pre-code pending: formalize 0.2–0.4 scripts in Setup Agent spec §6 + package the run as CAG example #4 + eval fixtures. Two lessons saved & auto-approved: "same words different maps" wizard catch (`076747df`) and situational-cut-beats-profession-cut for create_demand wedges (`d88bf6b5`). Open non-blocking flags: pre-launch cluster, Ad Library manual check, Capa-3 keyword volumes.
- D-028 (2026-06-10, DB `7c758914`) — **Sandi run 0.4 CLOSED + positioning gap locked.** Operator chose option (a): own category "guided multi-audience strategist that teaches while doing and closes the loop to money" (H1 multi-avatar-as-core + H2 guided full loop), executed in cycle 1 as "your way out of the marketplace, audience by audience" (H3, Priya), with pricing wedge ~$30 vs $99+ tier (H4). Never position as "AI marketing tool" (red ocean). Key scan finding: M1-Project charges multi-audience as upsell ($99=1 ICP, $199=3) with no loop — validates demand AND exposes the gap; substitute #1 = ChatGPT $20 (displaced by methodology+memory+loop, not price); Helium 10 killed its starter ($129 entry). Gate G-Phase-0.4 passed (5/5/5/5 with prices+sources; Ads channel inferred — Ad Library manual verification flagged, non-blocking). Rollup `product_brief_v2.json` built: ECONOMICS-001 passes by construction (LTV 500 / CAC 165 = 3.03, auto:ltv/3 — real test is Phase 4), 3 real refuted hypotheses (0 critical — healthy), research_metadata estimated (8h/$50). **Pending: operator's final signature on the rollup → closes global gate G-Phase-0 → fires build trigger D-024.**
- D-027 (2026-06-10) — **Sandi run 0.3 CLOSED — Gate G-Phase-0.3 passed.** Full set: primary persona (D-025), 4 avatars signed with per-avatar decision_records + 2-of-3 tests (Dana `40ac869a`, Marcus `f7de5d97`, Priya `ec84e43f`, Héctor `d8a873d2`), first-cycle prioritization = Priya (D-026), 2 negative personas declared with records (claims-imposibles `60a64765` ético / delegador-total `68d6b022` high-support-cost). `dmu.json` not required: hybrid with B2B parked — same hybrid-gate criterion signed in D-022/D-023 (gate evaluates lead_segment block). Pre-launch cluster explicitly NOT a negative (future wave / Module A). Next: 0.4 competitor scan (4 channels + ERRC verdict + pricing_tiers[]) → closes Phase 0 → fires build trigger D-024.
- D-026 (2026-06-10, DB `9f9bdb3d`) — **Sandi run 0.3: first cycle starts with Priya** (marketplace-caged avatar) — the spec's human prioritization decision. Operator's call, backed glass-box: hottest/timeable triggers (fee hikes, suspensions = rage windows with exact search), proven willingness-to-pay (sellers already buy tools), multi-audience by default (corporate gifts/home/weddings — the D-009 jewel demos perfectly), clean money attribution (own-channel vs marketplace revenue). Named risks: post-FBA-guru skepticism (demands fast honest proof), more solution-aware than the 0.2 wedge → create_demand applies as "marketplace independence playbook" (aim instruction for Phase 1-2). Also: operator added 4th avatar **Héctor (local route service, formalizing)** — closest pair with Dana, honest 2-of-3 JUSTO (distinct language/partially distinct channel; reinforced by recurring-revenue model); merge-by-evidence valve if Phase 2 content overlaps >70%. Héctor adds the high-digital-friction dimension (stresses adaptive onboarding) — good as 4th, wrong as 1st. Remaining for 0.3 gate: negative_personas + per-avatar decision_records at set signoff.
- D-025 (2026-06-10, DB `1b025704`) — **Sandi run 0.3: primary buyer persona signed.** "El experto en lo suyo que ya vende y está perdido en el marketing online" — artisan/expert identity (not seller), 6 sourced universal pains (CakeCentral literal quotes, LawnSite, Amazon Seller Forums, LocaliQ 2025 n=730+, confidence-gap aggregators), JTBD functional+emotional+social, 3 Empathy-Map incongruences as copy leverage. Signed unchanged by operator over research draft v1. Declared gaps: Reddit unreachable (robots), media_consumed mostly hypothesis, demographics wide pre-PMF. Artifact: `run/sandi/buyer_persona.json` (signed_v1).
