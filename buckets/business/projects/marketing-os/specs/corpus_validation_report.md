...
**Fecha:** 2026-06-09
**Last updated:** 2026-06-25 (freshness pass post-auditoria Fase 0/1 → Fase 2, ledger C1–C14)
...
> ### ⏱️ Freshness — estado de este reporte (2026-06-25)
> Este reporte fue redactado bajo la doctrina de oferta **anterior** a la simulacion de Papandi (value-stack Hormozi, valores percibidos por pieza, candado de ratio valor≥N×precio). Esa doctrina **fue retirada** por el registro de cambios C1–C14 (ver `_audit_change_ledger.md`) y reemplazada por la doctrina **v1.8: valor por FUNCIONALIDAD**. Las secciones que todavia describen el modelo viejo (Addendum 2026-06-11, fila §4 C2, enriquecimientos E2/E6) se conservan como **registro historico** pero quedan **anotadas SUPERSEDED** in-line. Dos puntos donde el reporte *endosaba activamente* el modelo viejo (la fila C2 §4 y el punto 3 del Addendum) fueron **corregidos**. Ademas se anaden abajo dos **divergencias intencionales** (C1, C4) como decisiones trazables — no son contradicciones a resolver, son rumbos firmados que se apartan a proposito del corpus.

---

## 0. Divergencias INTENCIONALES con el corpus (decisiones trazables, NO contradicciones)

> El corpus es el **piso**, no la autoridad final. Estas dos divergencias son rumbos **firmados a proposito** que se apartan del corpus; se documentan aqui para que sean **decisiones trazables, no omisiones silenciosas**. Ninguna se "resuelve" — gobiernan.

- **DIV-1 (C1) — Se RETIRA el value-stack de Hormozi y el candado de ratio.** El corpus (y la doctrina previa de este mismo reporte) justificaba cada pieza de la oferta por su **valor percibido** (`perceived_value_usd`, `comparable`, `value_rationale`) y exigia un **candado de ratio** (valor total ≥ N× precio, ej. 5×). **Eso fue retirado** (decision `a2ef7e37`). Ahora cada pieza se justifica por lo que **HACE / le permite al cliente** y por el **freno que quita**, nunca por comparacion con un valor externo. Comparar mete al producto en la sombra de otros (suena caro/barato) e introduce competidores — viola el glass-box. La "prueba de que vale" pasa a ser el **movimiento del eje debil** (`proposed_rescore`, p.ej. likelihood 3→6), no un ratio. El unico candado economico que queda es el **margen por forma de entrega** (`delivery_format`: digital 70% / servicio 50% / fisico 30% / hibrido 40%).
- **DIV-2 (C4) — Se RECHAZA la prueba social como mecanismo de la oferta.** El corpus de RRSS/conversion usa casos, testimonios y "a otros les funciono" como motor de credibilidad. **Para producto pre-lanzamiento eso esta prohibido** (decision `11b919ab`): sin clientes todavia, un caso/testimonio/simulacion seria una **claim fabricada**. El eje debil se ataca por **MECANISMO** ("por que te funciona a TI", anclado en los diferenciadores firmados) + **reversion de riesgo accionable** (prueba gratis, acompanamiento), nunca por prueba social. La creencia se construye por **experiencia propia**, no ajena. Gobierna ambos generadores (el `weak_plan_note` de 1.1 y el `offer-stack-builder` de 1.2) y el enfasis del contenido de Fase 2.
...
El spec **invierte/omite** esto: colecta `pain_points_universal` en 0.3 **sin producir artefacto de propuesta de valor en Phase 0** (el value framing se difiere a Phase 1). *(Corregido 2026-06-25: la redaccion original decia "Phase 1, Hormozi por avatar" — endosaba dos cosas ya retiradas. (a) El value framing de Phase 1 **ya no es Hormozi/value-stack** sino valor por FUNCIONALIDAD, candado = margen por `delivery_format` (C1, decision `a2ef7e37`). (b) La oferta **NO es por avatar**: es del **buyer persona** (una sola, compartida por todos sus avatares); lo que es por avatar es la estrategia/contenido de Fase 2 (C5). El value framing se reubica a Phase 1 **sobre el buyer persona**, no per-avatar.)*
...
> **NOTA DE FRESHNESS (2026-06-25):** este addendum valido la **v1.6** de Phase 1, que aun corria bajo la doctrina value-stack Hormozi + candado de ratio. Esa doctrina **fue retirada por v1.8** (decisions `a2ef7e37` y `11b919ab`; ledger C1–C4). El addendum se conserva como registro historico; los puntos que endosaban el modelo viejo quedan anotados SUPERSEDED in-line abajo (alineacion #3 corregida en su lugar; alineacion #7 y enriquecimientos E2/E6 anotados). Las divergencias intencionales que ahora gobiernan estan en la **§0** de este reporte.
...
3. **Margin gate (1.2) = Curso 1 "CPA < margen" como condicion inamovible de viabilidad.** El spec lo ancla en un **unico candado economico: el margen por forma de entrega** (`delivery_format`: digital 70% / servicio 50% / fisico 30% / hibrido 40%; el LLM clasifica el formato, el codigo mapea el umbral via `MARGIN_TARGETS`). La "prueba de que vale" no es economica sino el **movimiento del eje debil** (`proposed_rescore`, p.ej. likelihood 3→6). *(Corregido 2026-06-25: la redaccion original decia "Margin gate + ratio gate ... dos gates simultaneos (ratio percibido + margen bruto)". El **candado de ratio percibido fue retirado** (C1/C2, decision `a2ef7e37`): comparar contra un valor percibido mete competidores y viola el glass-box, y un 70% fijo no generalizaba a fisico/servicio. Queda **un solo gate**, el de margen por `delivery_format`.)*
...
7. **Compra reflexiva vs impulsiva (C6) = tabla `ratio_target` por business_type × purchase_type.** El corpus da la distincion; el spec la convierte en multiplicador de pricing [Context-Adjusted Threshold]. *(SUPERSEDED por v1.8 / decision `a2ef7e37`: la tabla `ratio_target` y el candado de ratio valor≥N×precio se retiraron. La distincion compra reflexiva/impulsiva sobrevive como senal cualitativa para la urgencia honesta (1.3), no como multiplicador de pricing.)*
...
| E2 | **Checklist de los 11 elementos de valor de Osterwalder** como lente de diseno del stack (detectar valor no cobrado; vocabulario candidato para `hormozi_category` custom → promocion Pattern A). Hormozi escribio sus 5 semillas para SUS productos; Osterwalder enumero el espacio. *(SUPERSEDED por v1.8 / decision `a2ef7e37`: el value-stack Hormozi y el campo `hormozi_category` se retiran. La lente "que valor no estoy entregando/cobrando" sobrevive, pero ahora cada pieza se justifica por lo que HACE y por el freno que quita, no por una categoria de valor percibido.)* | §5 | BMC §2.2 |
...
| E6 | **Nota de palanca barata en §0** (C5: optimizar propuesta de valor sube CR sin subir gasto de adquisicion) + **mineria de resenas como fuente complementaria** (C5: "propuestas ocultas" en reviews). *(PARCIALMENTE SUPERSEDED por v1.8 / decision `a2ef7e37`: el campo `value_rationale` se retira con el value-stack; la mineria de resenas sigue siendo fuente valida, pero alimenta el **mecanismo** — que freno quita / por que le funciona a TI — y los diferenciadores firmados, no un `value_rationale` comparativo. La nota de palanca barata en §0 se mantiene.)* | §0, §5 | Curso 2 C5 |

Tambien corregida una **errata cosmetica interna**: el JSON ejemplo de pricing decia `anchor_statement: "...$480...por $97"` junto a `your_price_usd: 47` — ejemplos incoherentes entre si en un artefacto que sirve de CAG. Armonizado a $235/$47 (ratio 5×, coherente con la fila B2C reflexiva de la tabla). *(SUPERSEDED por v1.8 / decision `a2ef7e37`: el `anchor_statement` que ancla precio contra un valor total/comparable se retira junto con el value-stack y el candado de ratio 5×. El statement de venta (1.4) presenta UNA linea de precio por funcionalidad, sin ancla comparativa.)*
