...
| **D-IT5** | Fuente de la capa civil (base) | **Librería/API de holidays por país** (recurrencia + variantes regionales ya resueltas; *siempre actualizada* — requisito del operador), **envuelta tras un adapter `HolidaySource`** para poder cambiar de proveedor. Default **`python-holidays`** (el runtime es Python) · **Nager.Date** como API alterna (gratis, open-source, 100+ países) · **Calendarific** si se necesita más cobertura/idiomas (de pago). Los **momentos comerciales/marketing** los seguimos curando nosotros (las librerías no los traen). Pick exacto (versión/cobertura/licencia) se confirma en la Trinity. |
